...
function createCandidateCard(candidate) {
  ...
  card.innerHTML = `
    ...
    <p>${member.yearsExperience != null ? `${member.yearsExperience} years experience` : 'Experience pending'}</p>
    <p>${completedDays.length ? `${completedDays.length} completed days` : 'Learning path pending'}</p>
  `;
...
function renderCandidateSummary() {
  ...
  const completedDays = missions.filter((mission) => mission.passed).map((mission) => mission.day);
  const summaryText = `${member.name || 'Candidate'} is a ${member.jobRole || 'professional'} with ${member.yearsExperience != null ? member.yearsExperience : 'a growing'} years of experience. Their learning profile highlights ${completedDays.length} completed milestones and strong momentum across the curriculum.`;
  ...
  elements.summaryExperience.textContent = member.yearsExperience != null ? `${member.yearsExperience} years` : 'Pending';
  elements.summaryDays.textContent = completedDays.length ? completedDays.slice(0, 6).join(', ') : 'None';
  elements.summaryText.textContent = `${summaryText}${member.education ? ` Education: ${member.education}.` : ''}`;
...
function resetInterviewView() {
  ...
  elements.progressLabel.textContent = 'Question 0 of 10';
...
function renderProgress() {
  const count = Math.min(Math.max(state.messageCount, 0), 10);
  const percent = Math.round((count / 10) * 100);
  elements.progressLabel.textContent = `Question ${count} of 10`;
...
function renderFeedback(feedback) {
  if (!feedback) {
    elements.feedbackContent.innerHTML = '<div class="feedback-card"><h3>No feedback available</h3><p>The interview ended without structured feedback.</p></div>';
    return;
  }

  const cards = [
    { title: 'Summary', items: [feedback.summary] },
    { title: 'Strengths', items: feedback.strengths || [] },
    { title: 'Gaps', items: feedback.gaps || [] },
    { title: 'Next', items: feedback.next || [] },
  ];

  elements.feedbackContent.innerHTML = cards.map((card) => {
    const items = card.items.length ? card.items : ['None recorded'];
    const list = items.map((item) => `<li>${item}</li>`).join('');
    return `<div class="feedback-card"><h3>${card.title}</h3><ul>${list}</ul></div>`;
  }).join('');
}

async function loadCandidates() {
  elements.backendStatus.textContent = 'Loading candidates…';
  try {
  ...
  try {
    const requestBody = {
      ...
      candidate: {
        ...
        completedDays: (state.selectedCandidate.missions || []).filter((mission) => mission.passed).map((mission) => mission.day),
    ...
    state.messageCount = 0;
    showInterviewView();
    elements.interviewTitle.textContent = `Interview with ${state.selectedCandidate.member?.name || 'candidate'}`;
    appendMessage('system', 'Interview started. The interviewer will ask a series of technical questions.');
    appendMessage('assistant', payload.reply || 'Welcome! Let us begin.');
    elements.formHint.textContent = 'Submit your first answer to continue the interview.';
    renderProgress();
  } catch (error) {
...
async function submitAnswer(event) {
  ...
  if (state.isLoading || !state.currentInterviewStarted || !state.sessionId) return;
  ...
  try {
    ...
    if (payload.done) {
      ...
      elements.formHint.textContent = 'The interview is complete. You can start another one.';
    } else {
      elements.answerInput.value = '';
      elements.answerInput.disabled = false;
      elements.formHint.textContent = 'Answer the next question when you are ready.';
    }
  } catch (error) {
